...
def validate(headers: Iterable[str]):
    ...
    for header_raw in headers:
        ...
        if matched := only_day_number_header_regex.match(header):
            ...
            (day_one_number, day_two_number,
             day_one, month_one, year_one,
             day_two, month_two, year_two) = map(int, matched.groups())
...
validate(get_headers((Path(__file__).parent / "docs/Tagebuch.md").read_text("utf-8")))
